...
function patchTimer1(window, setName, cancelName, nameSuffix) {
    var setNative = null;
        ...
        function timer1() {
            try {
                ...
                if (typeof data.handleId === NUMBER) {
                    // in non-nodejs env, we remove timer1Id
                    // from local cache
                    delete tasksByHandleId[data.handleId];
                }
                else if (data.handleId) {
                    // Node returns complex objects as handleIds
                    // we remove task reference from timer1 object
        ...
        patchMethod(window, setName, function (delegate) { return function (self, args) {
            if (typeof args[0] === FUNCTION) {
                ...
                if (typeof handle === NUMBER) {
                    ...
                    // reference in timer1Id Object for clearTimeout
...
Zone.__load_patch('timer1s', function (global, Zone, api) {
    var set = 'set';
    var clear = 'clear';
    patchTimer1(global, set, clear, 'Timeout');
    patchTimer1(global, set, clear, 'Interval');
    patchTimer1(global, set, clear, 'Immediate');
});
Zone.__load_patch('requestAnimationFrame', function (global, Zone, api) {
    patchTimer1(global, 'request', 'cancel', 'AnimationFrame');
    patchTimer1(global, 'mozRequest', 'mozCancel', 'AnimationFrame');
    patchTimer1(global, 'webkitRequest', 'webkitCancel', 'AnimationFrame');
});
